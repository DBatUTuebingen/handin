  ...
  (let* ([hi (assignment<->dir dir)]
    ...
    `(tr ([class ,(string-append (if firstinparentdir? "lastrow " "") (if active? "active" "inactive"))])
       ...
       (td ,(if (and team (not (string=? (path->string team) user))) (format "Team ~a:" team) "")
           ,(handin-link k user as-tutor hi (and (xor active? as-tutor) upload-suffixes))
...
(define (all-status-page user as-tutor)
  ...
  (define (diff-parent? dirs) (not (equal? (parent-dir (car dirs)) (parent-dir (cadr dirs)))))
  (define (zipped dir) (if (empty? dir) dir (map list dir (append (list-tail dir 1) (list #f)))))
  (define (map-group f dir) (map (lambda (dirs) ((f (diff-parent? dirs)) (car dirs))) (zipped dir)))
